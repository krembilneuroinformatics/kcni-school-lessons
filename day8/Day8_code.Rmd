...
```{r}
netdf <- melt(lower.tri.remove(networkW)) # matrix is symmetrical, we designate half of it at NA with lower.tri.remove
names(netdf)[1:2] <- c("sub1","sub2")
netdf$sub1 <- str_pad(netdf$sub1,4,"left","0") # the melting procedure has turned our character strings into numeric, fix
netdf$sub2 <- str_pad(netdf$sub2,4,"left","0")
netdf <- netdf[complete.cases(netdf),]

netdf_sub <- subset(netdf, value < 0.5 & sub1!=sub2)
