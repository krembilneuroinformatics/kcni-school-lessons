...
head(pheno_long)
```
OK mabye Dan is right that in the tidyverse we need to do two steps to go from our "semi" long version to the wide version we want..
...
Here's how Dan would have done it with reshape

```{r, eval = FALSE}
pheno_wide1 <- reshape(pheno_long, 
                      direction='wide',
                      timevar="time",
...
## create a small function that set a random selection subset of the data to NA
set_random_nas <- function(x,frac) {
  x[sample( c(1:length(x)), floor(frac*length(x)))] <- NA; x}


## apply this function to all columns of our data but 
pw_miss <- pheno_wide %>%
  mutate_at(vars(-`id`, -dx, -msex),
            .funs = ~set_random_nas(.x,percent_missing))
...
str_c("Missing observations remaining = ",sum(is.na(newdat)))
```
...
```{r}
ggplot(pheno_wide, aes(age_0, age_1)) + geom_point()
```
...
```{r}
# let's simplify things going forwared and create one age variable, representing age at baseline (age.0)
...
meth <- meth %>% select(colnames(expr))
eeg <- eeg %>% select(colnames(expr))
...
dataL <- list(expr=t(expr),
              eeg=t(eeg),
              meth=t(meth))

dataLnorm = dataL %>% map(standardNormalization)

t(expr[1:5,1:5])
dataLnorm$expr[1:5,1:5]
```

### 3. Calculate subject-wise adjacency matrices (networks)
```{r}
## calculate the data distances with dist2
distL = dataLnorm %>%
  map(function(x) (dist2(x, x))^(1/2)) 

## report the dimensions of each list
map(distL,dim)

## calculate affinity matrixes from the distances
affinityL = distL %>%
  map(function(x) affinityMatrix(x, K, alpha))
```
...
```{r}
# joining the cluster info bay to the original pheno data
pheno <- inner_join(pheno_wide,clusters,by = c("id"="phenoid"))

dx_counts <- pheno %>% count(dx,clust4)
...
```{r}
# a simple way to run a chi-squared test
chisq.test(x = pheno$dx,
           y = pheno$clust4)
```
```{r}
# a convoluted way to do a chisq test without table or $ 
dx_counts %>%
  pivot_wider(names_from = dx,
              values_from = n,
              values_fill = list(n=0)) %>%
...
```{r}
ggplot(data=dx_counts, aes(x=as.factor(clust4), y=n, fill=dx)) +
...
```{r}
## using the SNF rankFeatureByNMI function to to get NMI - or Normalized Mutual Information 
rankedfeatures <- rankFeaturesByNMI(dataLnorm, networkW) %>%
  set_names(c("NMI","rank"))

## converting from the list format back to something more tidy so we view it more easily
rankedfeatures_tidy <- rankedfeatures %>% 
  as_tibble() %>%
  mutate(datatype = names(dataL)) %>%
  unnest(c("NMI", "rank")) %>%
  mutate(data_col = c(colnames(dataLnorm$expr), 
                      colnames(dataLnorm$eeg), 
                      colnames(dataLnorm$meth)))
```
...
pheno2 <- dataLnorm %>% 
  pluck("meth") %>% 
  as.tibble(rownames = "snfid") %>%
  inner_join(pheno, by = "snfid")
