...
# Part 1: For the rstudio environment (Days 1, 2, 3, 7 & 8)
...
- **SciNet Password**: should get this with your username
    - referred to in code sections below as `<password>`
- **Your random rstudio password**: this will be generated by the start up script and printed to the screen
- **Your very own port number**: this will also be selected at random when you run scripts and printed to the screen.
...
```sh
source /scinet/course/ss2020/5_neuroimaging/setup_scinet_kcni_env_part1.sh
```
...
To do this - we have created a custom script to source.. it will also generate `<your-port>``<your-rstudio-password>`).

```sh
source /scinet/course/ss2020/5_neuroimaging/setup_scinet_kcni_rstudio_env_part2.sh 
```
...
It will prompt for a username and password. This will be `<your-username>` and `<your-rstudio-password>`
(Note these will have been printed to the terminal after you run Step 1.3)
...
```sh
mv $SCRATCH/kcni-school-lessons $SCRATCH/kcni-school-lessons1
source /scinet/course/ss2020/5_neuroimaging/setup_scinet_kcni_env_part1.sh
```
...
#### the rstudio docker was created by converting from docker with docker2singularity

```sh
docker run -v /var/run/docker.sock:/var/run/docker.sock -v C:\Users\Erin_Dickie\code:/output --privileged -t --rm quay.io/singularity/docker2singularity edickie/kcnischool-rstudio:latest
```

This created the file..

edickie_kcnischool-rstudio_latest-2021-07-03-20331903aa75.sif